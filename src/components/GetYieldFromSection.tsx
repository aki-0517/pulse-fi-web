const GetYieldFromSection = () => {
  ...
  return (
    <section id="get-yield-from" className="py-32 bg-gray-50 overflow-hidden">
      ...
      <div className="relative w-full">
        <div className="flex animate-marquee">
          {duplicatedYieldSources.map((source, index) => (
            ...
            >
              <div
                className="bg-white rounded-full w-48 h-48 flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2"
              >
                <img src={source.logoUrl} alt={`${source.name} logo`} className="w-24 h-24" />
              </div>
              <h3 className="mt-4 text-2xl font-bold text-gray-900">{source.name}</h3>
            </div>
          ))}
        </div>
      </div>
       <div className="text-center mt-16">
          <p className="text-gray-600 text-lg">and more...</p>
        </div>
    </section>
  );
};
