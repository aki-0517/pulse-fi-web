...
const RetroEffect = forwardRef<RetroEffectImpl, { colorNum: number; pixelSize: number }>(
  function RetroEffectComponent(props, ref) {
    const { colorNum, pixelSize } = props;
    const WrappedRetroEffect = wrapEffect(RetroEffectImpl);
    return (
      WrappedRetroEffect && (<WrappedRetroEffect ref={ref} colorNum={colorNum} pixelSize={pixelSize} /> as any)
    );
  }
);
...
function DitheredWaves({
  ...
  return (
    <>
      <mesh ref={mesh} scale={[viewport.width, viewport.height, 1] as [number, number, number]}>
        <planeGeometry args={[1, 1] as [number, number]} />
        <shaderMaterial
      ...
      {/* EffectComposer/RetroEffectの型エラーを回避 */}
      {/* @ts-expect-error 型の不一致を許容 */}
      <EffectComposer>
        {/* @ts-expect-error 型の不一致を許容 */}
